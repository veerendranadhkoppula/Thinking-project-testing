...
export async function POST(req: Request) {
  try {
    ...
    const { pageLinkIndex, pageLink, threadId, commentId, message } = await req.json()

    if (!websiteId || !threadId || !commentId || typeof message !== 'string') {
    ...
    if (!comments.length)
      return NextResponse.json({ error: 'No comments to edit' }, { status: 409 })
    ...
    if (!lastAuthorEmail || lastAuthorEmail !== serverUserEmail) {
      return NextResponse.json(
        { error: 'Not allowed: only the original author can edit their latest comment' },
        { status: 403 },
      )
    }

    if (last?.edited) {
      return NextResponse.json({ error: 'Comment was already edited once' }, { status: 409 })
    }
    ...
    return NextResponse.json({
      success: true,
      thread,
      editedComment: comments[lastIdx],
